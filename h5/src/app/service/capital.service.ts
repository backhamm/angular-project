...
import {CommissionParams, GameList, InviteList, ResponseBody, ResponseList, WithDraw, WithdrawParams, ProxyImage, ProxyInfo} from '@src/app/types/common.type';
...
export class CapitalService {
   ...
   */
  getProxyImage() {
    this.request.get('unauthor/agency/config/info').subscribe(res => {
      if (res.status === 10000) {
        this.proxyImage = res.data;
      }
    });
  }

  // 无限代理用户信息
  getProxyInfo(callback = null) {
    this.request.get('agency/user/info').subscribe(res => {
      if (res.status === 10000) {
  ...
  joinAgent(): Observable<ResponseBody> {
    return this.request.get('agency/join/agency');
  }

  // 获取无限代理邀请方式
  getAgentInvited(): Observable<ResponseBody> {
    return this.request.get('agency/invited/method');
  }

  // 获取无限代理团队图表信息
  getTeamChartInfo(): Observable<ResponseBody> {
    return this.request.get('agency/team/chart/info');
  }

  // 获取无限代理团队图表信息
  getCommissionChartInfo(): Observable<ResponseBody> {
    return this.request.get('agency/commission/chart/info');
  }

  // 获取代理公告
  getAgencyCarousel(): Observable<ResponseBody> {
    return this.request.get('unauthor/agency/carousel/record');
  }

  // 获取邀请记录
  getInvitedRecord(params): any {
    return this.request.get('agency/invited/record', params);
  }

  // 获取团队管理
  getTeamManagement(params): any {
    return this.request.get('agency/team/manage/info', params);
  }

  // 获取会员佣金流水详情
  getBrokerageDetail(params): any {
    return this.request.get('agency/commission/detail', params);
  }

  // 获取提佣流水
  getBrokerageRecord(params): any {
    return this.request.get('agency/commission/info', params);
  }

  // 获取提佣记录
  getExtractRecord(params): any {
