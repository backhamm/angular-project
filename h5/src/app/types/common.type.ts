...
export interface ProxyImage {
  userCenterBanner: string;
  ruleBanner: string;
  agencyPromote: string;
  introducePromote: string;
  introduceBanner: string;
  commissionMapImg: string;
  promotionCourse: string;
  commissionConfig: {
    id: number;
    cagent: string;
    minCycle: number;
...
export interface ProxyInfo {
  ...
  teamNum: number;
  totalNum: number;
  directAgencyNum: number;
  directAgencyAddNum: number;
  directAddNum: number;
  teamAddNum: number;
  ...
  yesterdayDirectBetAmount: number;
  yesterdayTeamBetAmount: number;
...
export interface UserInfo {
  ...
  hasJoin: number;             // 代理状态    0-加入 1-停用 2-未加入
