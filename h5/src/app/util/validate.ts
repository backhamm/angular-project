...
export const _Reg: any = {
  ...
  password: {
    reg: /^[0-9A-Za-z]{6,12}$/, // 密码
    message: '密码由6-12位字母或数字组合',
    placeholder: '密码(6-12位数字与字母组合)'
  },
  recommendCode: {
...
export class Validate {
  constructor(
    ...
    this.singleValidateFn = this.singleValidateFn.bind(this);
    this.mulValidateFn = this.mulValidateFn.bind(this);
    this.amount_validate = this.amount_validate.bind(this);
  }
  ...
  // 单笔限额
  amount_validate(min: any, max: any, amount): Boolean {
    if (!amount || amount > max || amount < min) {
      this.message.error(`单笔限额${min}-${max}(元)`);
      return false;
    }
    return true;
  }
